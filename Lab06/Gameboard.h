...
class Gameboard
{
	...
	friend int main(); // for unit test purposes
public:
	// CONSTANTS
...
public:	
	...
	int getContent(Point block) const;
	...
	void setContent(Point block, int content);
	...
	void setContent(std::vector<Point>& points, const int content);
...
private:  // This is commented out to allow us to test. 
	...
	bool isValidPoint(const Point& block) const;
	...
	// - return: nothing
	void copyRowIntoRow(const int sourceIndex, const int targetIndex);
